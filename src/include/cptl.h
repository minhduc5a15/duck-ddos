...
namespace ctpl {
    ...
    public:
        ...
        explicit thread_pool(const int nThreads, const int queueSize = _ctplThreadPoolLength_) : q(queueSize) {
            this->init();
        ...
        std::thread &get_thread(const int i) const { return *this->threads[i]; }
        ...
        void resize(const int nThreads) {
            if (!this->isStop && !this->isDone) {
                const int oldNThreads = static_cast<int>(this->threads.size());
        ...
        std::function<void(int)> pop() const {
